...
class Keyboard {
     ...
     */
    clone() {
        const keyboard = this.constructor.make(this.buttons).setOptions(this.options)
    ...
    combine(...keyboards) {
        ...
        keyboards.forEach(keyboard => {
            if (keyboard instanceof this.constructor) {
    ...
    rebuild(...args) {
        return this.make(this.makeFunction(...args))
    }
    ...
    // static methods
    static remove() {
        return new this().resize(false).removeKeyboard().draw()
    }

    static make(buttons, makeOptions = {}) {
        return new this().make(buttons, makeOptions)
    }

    static combine(...keyboards) {
        return new this().combine(keyboards)
    }
    ...
    static inline(buttons, makeOptions = {}, extraMarkup = {}) {
        if (buttons instanceof this) {
            return buttons.inline()
        }

        return this.make(buttons, makeOptions).inline(extraMarkup)
    }
    ...
    static reply(buttons, makeOptions = {}, extraMarkup = {}) {
        ...
        return this.make(buttons, makeOptions).reply(extraMarkup)
    }
    ...
    static builtIn(buttons, makeOptions = {}, extraMarkup = {}) {
        return this.reply(buttons, makeOptions, extraMarkup)
    }
}
